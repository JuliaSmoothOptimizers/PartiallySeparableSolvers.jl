...
@testset  "P- SR1/BFGS " begin
	...
	f_approx = ( elm_fun :: PartiallySeparableNLPModels.element_function -> PartiallySeparableNLPModels.element_hessian{Float64}( Array{Float64,2}(zeros(Float64, length(elm_fun.used_variable), length(elm_fun.used_variable)) ) ) )
	f = (y :: PartiallySeparableNLPModels.element_function -> PartiallySeparableNLPModels.element_gradient{typeof(x[1])}(Vector{typeof(x[1])}(undef, length(y.used_variable) )) )

	exact_Hessian = PartiallySeparableNLPModels.Hess_matrix{Float64}(f_approx.(Struct_PS.structure))
	approx_hessian_SR1 = PartiallySeparableNLPModels.Hess_matrix{Float64}(f_approx.(Struct_PS.structure))
	approx_hessian_BFGS = PartiallySeparableNLPModels.Hess_matrix{Float64}(f_approx.(Struct_PS.structure))
	...
	sp_approx_BFGS = PartiallySeparableNLPModels.construct_Sparse_Hessian(Struct_PS, approx_hessian_BFGS)

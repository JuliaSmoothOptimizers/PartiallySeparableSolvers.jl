...
@testset "instantiation new version" begin
		...
		ex_mathopt = CalculusTreeTools.transform_to_expr_tree(obj_Expr) 

		ModelingToolkit.@variables x[1:n]
		fun = ros_adnlp.f(x)
		ex_ad = CalculusTreeTools.transform_to_expr_tree(fun)
	...
	@testset "ADNLPModel/MathOptNLPModel" begin
		...
		@test CalculusTreeTools.evaluate_expr_tree(ex_2, x0) == CalculusTreeTools.evaluate_expr_tree(ex_1, x0)
		@test n_1 == n_2
		@test x0_1 == x0_2

		pd_pbfgs1 = build_PartitionedData_TR_PBFGS(ex_1, n; x0)
		pd_pbfgs2 = build_PartitionedData_TR_PBFGS(ex_2, n; x0)

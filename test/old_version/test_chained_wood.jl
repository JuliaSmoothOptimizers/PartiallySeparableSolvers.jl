function create_chained_wood_JuMP_Model(n::Int)
  ...
  @NLobjective(
    ...
    Min,
    sum(
      100 * (x[2 * j - 1]^2 - x[2 * j])^2 +
      (x[2 * j - 1] - 1)^2 +
      90 * (x[2 * j + 1]^2 - x[2 * j + 2])^2 +
      (x[2 * j + 1] - 1)^2 +
      10 * (x[2 * j] + x[2 * j + 2] - 2)^2 +
      (x[2 * j] - x[2 * j + 2])^2 * 0.1 for j = 1:Integer((n - 2) / 2)
    )
  ) #rosenbrock function
...
@testset " test convexity detection chainedwood" begin
  for i = 1:length(Struct_PS.structure)
    if (i % 6 == 1) || (i % 6 == 3)
      @test CalculusTreeTools.is_unknown(
        PartiallySeparableNLPModels.get_convexity_status(Struct_PS.structure[i]),
      )
    else
      @test CalculusTreeTools.is_convex(
        PartiallySeparableNLPModels.get_convexity_status(Struct_PS.structure[i]),
      )
    end
  end

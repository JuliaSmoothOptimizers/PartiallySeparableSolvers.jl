...
# création du modèle 
function create_convex_JuMP_Model(; nb_inter::Integer = 100, a = 0, b = 1, _string = "undefined")
  m = Model()
  ...
  @NLobjective(
    m,
    Min,
    sum((x[j - 1] - x[j + 1])^2 / (8 * h) for j = 2:Integer(n - 1)) +
    (x[2] - a)^2 / (8 * h) +
    (b - x[n - 1])^2 / (8 * h) +
    (x[1] - a)^2 / (4 * h) +
    (b - x[n])^2 / (4 * h)
...
H_pbsr1 = zeros(n, n)
H_pbs = zeros(n, n)
H_pbfgs = PartiallySeparableNLPModels.construct_full_Hessian(
  sps_pbfgs.sps,
  sps_pbfgs.tpl_B[Int(sps_pbfgs.index)],
)
H_psr1 = PartiallySeparableNLPModels.construct_full_Hessian(
  sps_psr1.sps,
  sps_psr1.tpl_B[Int(sps_psr1.index)],
)
H_pbs =
  PartiallySeparableNLPModels.construct_full_Hessian(sps_pbs.sps, sps_pbs.tpl_B[Int(sps_pbs.index)])
...
function create_convex_separable_JuMP_Model(; n::Integer = 10, a = 0, b = 1, _string = "undefined")
  m = Model()
  @variable(m, x[1:n])
  mod(n / 2, 2) != 0 && error("n n'est pas un multiple de 2")
  @NLobjective(m, Min, sum((5 * x[2 * j - 1] + x[2 * j])^2 for j = 1:Integer(n / 2)))
  evaluator = JuMP.NLPEvaluator(m)
  MathOptInterface.initialize(evaluator, [:ExprGraph, :Hess])
  obj = MathOptInterface.objective_expr(evaluator)
  vec_var = JuMP.all_variables(m)

...
function alloc_struct_algo(obj::T, n::Int, type = Float64::DataType) where {T}
  ...
  construct_element_grad = (
    y::PartiallySeparableNLPModels.element_function ->
      PartiallySeparableNLPModels.element_gradient{type}(
        Vector{type}(zeros(type, length(y.used_variable))),
      )
  )
  ...
  construct_element_hess = (
    elm_fun::PartiallySeparableNLPModels.element_function ->
      PartiallySeparableNLPModels.element_hessian{type}(
        Array{type, 2}(undef, length(elm_fun.used_variable), length(elm_fun.used_variable)),
      )
  )
...
function init_struct_algo!(
  ...
  PartiallySeparableNLPModels.evaluate_SPS_gradient!(
    s_a.sps,
    s_a.tpl_x[Int(s_a.index)],
    s_a.tpl_g[Int(s_a.index)],
  )
  ...
  s_a.tpl_f[Int(s_a.index)] =
    PartiallySeparableNLPModels.evaluate_SPS(s_a.sps, s_a.tpl_x[Int(s_a.index)])
  s_a.n_eval_obj += 1
end
...
function approx_quad(s_a::struct_algo{T, Y}, x::AbstractVector{Y}) where {T} where {Y <: Number}
  s_a.tpl_f[Int(s_a.index)] +
  s_a.grad' * x +
  1 / 2 * PartiallySeparableNLPModels.product_matrix_sps(s_a.sps, s_a.tpl_B[Int(s_a.index)], x)' * x
end
...
function update_PSR1!(
  ...
  (s_k, info) = Krylov.cg(
    ...
    radius = s_a.Δ,
    itmax = max(2 * n, 50),
  )::Tuple{Array{Y, 1}, Krylov.SimpleStats{Y}}
  ...
  if ρₖ > s_a.η #= on accepte le nouveau point =#
    ...
    PartiallySeparableNLPModels.build_gradient!(s_a.sps, s_a.tpl_g[Int(s_a.index)], s_a.grad)
    PartiallySeparableNLPModels.minus_grad_vec!(
      s_a.tpl_g[Int(s_a.index)],
      s_a.tpl_g[Int(other_index(s_a))],
      s_a.y,
    )
    s_a.n_eval_grad += 1
    ...
    update_SPS_SR1!(
      s_a.sps,
      s_a.tpl_B[Int(other_index(s_a))],
      s_a.tpl_B[Int(s_a.index)],
      s_a.y,
      s_k,
    ) #on obtient notre nouveau B_k
...
iterations_TR_PSR1!(
  ...
  start_time::Float64;
  kwargs...,
) where {T} where {Y <: Number} = begin
  (cpt, elapsed_time) = iterations_TR_PSR1!(s_a, start_time; kwargs...)
  vec_cpt[1] = cpt
  return (vec_cpt[1], elapsed_time)
end
function iterations_TR_PSR1!(
  s_a::struct_algo{T, Y},
  start_time::Float64;
  max_eval::Int = 10000,
  ...
  opB(s::struct_algo{T, Y}) = LinearOperators.LinearOperator(
    Y,
    n,
    n,
    true,
    ...
    (
      (res, v) ->
        PartiallySeparableNLPModels.product_matrix_sps!(s.sps, s.tpl_B[Int(s.index)], v, res)
    ),
  )
  ...
  while (
    (norm(s_a.grad, 2) > s_a.ϵ) &&
    (norm(s_a.grad, 2) > s_a.ϵ * ∇fNorm2) &&
    s_a.n_eval_obj < max_eval
...
solver_TR_PSR1!(
  obj_Expr::T,
  n::Int,
  x_init::AbstractVector{Y},
  type = Float64::DataType;
  kwargs...,
) where {T} where {Y <: Number} = _solver_TR_PSR1!(obj_Expr, n, x_init, type; kwargs...)
function _solver_TR_PSR1!(
  obj_Expr::T,
  n::Int,
  x_init::AbstractVector{Y},
  type = Float64::DataType;
...
function _solver_TR_PSR1_2!(
  ...
  max_time::Float64 = 30.0,
  kwargs...,
) where {T} where {Y} where {Z <: AbstractNLPModel}
  (x_final, s_a, cpt, elapsed_time) =
    solver_TR_PSR1!(obj_Expr, n, x_init, type; max_eval = max_eval, max_time = max_time, kwargs...)
...
iterations_TR_PBFGS!(
  ...
  (cpt, elapsed_time) = iterations_TR_PBFGS!(s_a, start_time; kwargs...)
  vec_cpt[1] = cpt
  return (vec_cpt[1], elapsed_time)
end
function iterations_TR_PBFGS!(
  s_a::struct_algo{T, Y},
  start_time::Float64;
  max_eval::Int = 10000,
  max_time::Float64 = 30.0,
...
solver_TR_PBFGS!(
  ...
  type = Float64::DataType;
  kwargs...,
) where {T} where {Y <: Number} = _solver_TR_PBFGS!(obj_Expr, n, x_init, type; kwargs...)
function _solver_TR_PBFGS!(
  obj_Expr::T,
  n::Int,
  x_init::AbstractVector{Y},
  type = Float64::DataType;
...
function _solver_TR_PBFGS_2!(
  ...
  (x_final, s_a, cpt, elapsed_time) =
    solver_TR_PBFGS!(obj_Expr, n, x_init, type; max_eval = max_eval, max_time = max_time, kwargs...)
  nrm_grad = norm(NLPModels.grad(m, x_final), 2)
  nrm_grad_init = norm(NLPModels.grad(m, x_init), 2)
  if nrm_grad < nrm_grad_init * 1e-6 || nrm_grad < 1e-6
    status = :first_order
...
function _solver_TR_PBFGS!(
  ...
  x::AbstractVector = copy(model_JUMP.meta.x0),
  kwargs...,
) where {T <: AbstractNLPModel} where {Y <: Number}
  model = model_JUMP.eval.m
  evaluator = JuMP.NLPEvaluator(model)
  MathOptInterface.initialize(evaluator, [:ExprGraph])
  obj_Expr = MathOptInterface.objective_expr(evaluator)::Expr
...
iterations_TR_PBS!(
  ...
  (cpt, elapsed_time) = iterations_TR_PBS!(s_a, start_time; kwargs...)
  vec_cpt[1] = cpt
  return (vec_cpt[1], elapsed_time)
end
function iterations_TR_PBS!(
  s_a::struct_algo{T, Y},
  start_time::Float64;
  max_eval::Int = 10000,
  max_time::Float64 = 30.0,
...
solver_TR_PBS!(
  ...
  type = Float64::DataType;
  kwargs...,
) where {T} where {Y <: Number} = _solver_TR_PBS!(obj_Expr, n, x_init, type; kwargs...)
function _solver_TR_PBS!(
  obj_Expr::T,
  n::Int,
  x_init::AbstractVector{Y},
  type = Float64::DataType;
...
function _solver_TR_PBS_2!(
  ...
  (x_final, s_a, cpt, elapsed_time) =
    solver_TR_PBS!(obj_Expr, n, x_init, type; max_eval = max_eval, max_time = max_time, kwargs...)
  nrm_grad = norm(NLPModels.grad(m, x_final), 2)
  nrm_grad_init = norm(NLPModels.grad(m, x_init), 2)
  if nrm_grad < nrm_grad_init * 1e-6 || nrm_grad < 1e-6
    status = :first_order

...
function my_Part_Trunk(model_JUMP :: T; max_eval :: Int=10000, x :: AbstractVector=copy(model_JUMP.meta.x0), kwargs...)  where T <: AbstractNLPModel
...
function compute_ratio_no_mat(x :: AbstractVector{Y},
                      f_x :: Y, s :: Vector{Y},
                      nlp :: AbstractNLPModel,
...
Update the radius of the Trust region Δ.
"""
function upgrade_TR_LO_no_update!( pk :: Float64, # value of the ratio
...
Even if the name suggest LSR1, B can be a BFGS Operator.
"""
function solver_TR_CG_Ab_NLP_Trunk(nlp :: PartionnedNLPModel{T,Y};
    x :: AbstractVector=copy(nlp.meta.x0),
    max_eval :: Int=10000,
    atol :: Real=√eps(eltype(x)),
    rtol :: Real=√eps(eltype(x)),
